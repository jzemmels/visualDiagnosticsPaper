...
We consider a data set of 510 cartridge cases collected as part of a study by \citep{Baldwin} and scanned at the Roy J. Carver High Resolution Microscopy Facility at Iowa State University.
The researchers fired the Remington 9mm centerfire cartridge cases from 25 Ruger SR9 pistols.
...
We split the cartridge cases data set by randomly selecting all 210 cartridge cases from 10 of the 25 firearms for training and using the 300 cartridge cases from the remaining 15 firearms for testing.
We consider every pairwise comparison between these cartridge cases, meaning the training data consists of $\binom{210}{2} = 21,945$ comparisons and test data consists of $\binom{300}{2} = 44,850$ comparisons.

\subsection{Visual Diagnostic Statistics as Features}

Let $A$ and $B$ denote a pair of cartridge cases.
...
\jz{[More to say about this specific outlier? Perhaps regarding other characteristics?]}
