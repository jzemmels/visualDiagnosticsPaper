...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.8\textwidth]{figures/preProcessEffectExample.png}
...
In this section, we explore using the Visual Diagnostic Features discussed above as features to differentiate between matching and non-matching comparisons.
...
We see in the bar plot in top left corner that there are 19,756 non-matching comparisons to 2,199 matching comparisons.
...
The other visuals in the first row show box plots of the 9 features, which complement the density plots along the main diagonal of each feature as well as the un-normalized histograms in the first column.
...
On the other hand, the box plots in the first row provide a more succinct summary of rank statistics and outliers for each feature.
...
The lower-triangle visuals show scatterplots for each pair of features with each point representing a single pairwise comparison while the upper-triangle shows correlation summaries between each feature, also distinguished by ground-truth.
We see that the variables with the strongest relationship are the average and standard deviation of the cell-based similarities vs. differences ratio, $\bar{r}_{\text{cell}}$ and $s_{\text{cell},r}$ in the previous section, although their seemingly quadratic relationship is expected.
We see a similar, albeit more linear, relationship between the average and standard deviation of the cell-based region sizes.

There are also notable relationships between full scan and cell-based features.
For example, the relationship between full scan and cell-based different region correlations, $cor_{\text{full, diff}}$ and $cor_{\text{cell, diff}}$, is particularly strong for the matching compared to non-matching comparisons as evidenced by the starkly different correlations of 0.775 and 0.358, respectively.
...
On the other hand, we wouldn't expect the full scan and cell-based registrations to agree for a non-matching comparison, so different markings may be compared at the two scales.
...
\jz{[Dive into specific examples of scans here. Look into scans with low/high match probability and why (specific feature tipping the scales? what do they look like visually/using the comparison plot? connect numeric summary above to a qualitative discussion of specific pairs based on visual diagnostics)]}
