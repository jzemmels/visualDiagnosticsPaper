...
We can apply \autoref{alg:registration} in both directions to align not only scan $B$ to $A$, but also $A$ to $B$.
...
To accommodate these two comparison directions, we now introduce a subscript $d = A,B$ that refers to the source scan used in \autoref{alg:registration}.
...
The output of \autoref{alg:cellComparison} is a set of estimated registrations - one registration for each cell in the source scan for each $\theta$.
...
Similar to \autoref{alg:registration}, we can use \autoref{alg:cellComparison} to align not only cells from $A$ to $B^*$, but also cells from $B$ to $A^*$, which is an aligned version of scan $A$ from \autoref{alg:registration} using $B$ as source.
We again use a direction subscript $d = A,B$ to refer to the source scan in \autoref{alg:cellComparison}.
For example, the outcome of \autoref{alg:cellComparison} using $B$ as source and $A^*$ as target is the set $\pmb{F}_B$ of estimated registrations per cell, $t = 1,...,T_B$, per rotation, $\theta \in \pmb{\Theta}_B'$.

One challenge with using registration algorithms like \autoref{alg:registration} and \autoref{alg:cellComparison} is understanding when and how they "work" as expected.
...
Rarely do \textit{all} cells agree with the same registration in practice, even for truly matching cartridge cases.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.8\textwidth]{images/preProcessEffectExample.png}
...
\autoref{fig:averageFilterExample} shows the construction of a filtered element-wise average between $A$ and $B^*$.
We compute the element-wise average $\frac{1}{2}(A + B^*)$ and absolute difference $|A - B^*|$ as shown in the left and right of \autoref{fig:averageFilterExample}.
...
For example, the right side of \autoref{fig:averageFilterExample} shows the elements of matrix $|A - B^*| \leq 1$ with $TRUE$ elements represented as white pixels and $FALSE$ elements as black pixels.
We then filter $\frac{1}{2}(A + B^*)$ using $|A - B^*| \leq \tau$ as the $cond$ matrix, resulting in tge filtered element-wise average $\mathcal{F}_{|A - B^*| \leq \tau}\left(\frac{1}{2}(A + B^*)\right)$, an example of which is shown at in the bottom of \autoref{fig:averageFilterExample} using $\tau = 1$.
...
#   impressions::x3p_comparisonPlot(x3p1 = reference,x3p2 = target,threshold = 1,
#                                   plotLabels = c(" ","  ","   ","    ","     "),
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.6\textwidth]{images/filteringIllustration.png}
\caption{To construct the comparison plot after aligning the scans K013sA1 and K013sA2 shown in \autoref{fig:processedSideBySide}, we compute their element-wise average (left) and element-wise absolute difference (right). We then compute a Boolean-valued matrix based on whether the elements of the element-wise absolute difference is greater or less than 1 micron (right). We use this Boolean matrix to distinguish between similarities and differences in the scan impressions.}
...
\autoref{fig:differenceFilterExample} shows an example of the results of this filtering using $\tau = 1$.

\begin{figure}[!htbp]
\centering
\includegraphics[width=.55\textwidth]{images/filteringDifferencesIllustration.png}
\caption{To construct the comparison plot, we filter two scans $K013sA1$ and $K013sA2$ based on regions where their element-wise absolute difference exceeds 1 micron, which are represented as white pixels in the black and white image. This emphasizes regions where the two surfaces differ, which complements the similarities that are emphasized in the element-wise average shown in \autoref{fig:averageFilterExample}.}
\label{fig:differenceFilterExample}
\end{figure}
...
\autoref{fig:complementCompPlot_fullScan} shows an example of a comparison plot using $A=$ K013sA1 and $B=$ K013sA2.
...
In \autoref{fig:complementCompPlot_fullScan}, we use a filtering threshold of $\tau = 1$ microns and represent filtered elements in light gray (gray80).
...
The right column of \autoref{fig:complementCompPlot_fullScan} shows the filtered differences between scans $A$ and $B^*$ using a cutoff threshold of $\tau = 1$ microns.
...
To visually assess the similarity between these markings, we can "zoom in" to specific regions of the cartridge case surfaces using the cell-based comparison procedure outlined in \autoref{alg:cellComparison}.
...
\autoref{fig:complementCompPlot_cellBased} shows the comparison plot of cell 3, 8 from scan K013sA1 and its target mate in K013sA2.
...
Compared to the depiction of this region in \autoref{fig:complementCompPlot_fullScan}, it is much easier to identify similar and different markings using this zoomed-in visualization.
For example, the dark purple "dots" on the upper-left side of the two scans are more prominent in this visualization.
We note that the color scale mapping now ranges from -5 to 4.1 microns compared to -12.3 to 20.3 microns in \autoref{fig:complementCompPlot_fullScan}.
...
<<matchCCP_combine, cache=TRUE,include=FALSE>>=
...
ggsave(plot = patchComparisonPlts_cellBased,filename = "figures/complementCompPlot_cellBased.png",
       width = 7,height = 7,bg = "white")
knitr::plot_crop("figures/complementCompPlot_cellBased.png")
@


\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/complementCompPlot_cellBased.png}
...
\label{fig:complementCompPlot_cellBased}
\end{figure}


<<eval=TRUE,cache=TRUE,include=FALSE>>=
K013sA1 <- x3ptools::x3p_read("data/K013sA1_processed.x3p")
...
K002eG1 <- x3ptools::x3p_read("data/K227iG3_processed.x3p")
...
ggsave(plot=nonMatch_similarCell_plt[[1]],filename="images/nonMatch_similarCell.png",width = 10,height = 6,bg = "white")
# knitr::plot_crop("figures/nonMatch_similarCell.png")
@
...
ggsave(plot = similarNonMatchCells_pltLabeled,
       filename = "images/similarNonMatchCells_pltLabeled.png",
       bg = "white",width = 10,height = 5)
knitr::plot_crop("images/similarNonMatchCells_pltLabeled.png")
@


The comparison plot is particularly useful for understanding the results of the cell-based registration procedure outlined in \autoref{alg:cellComparison}.
For example, the top of \autoref{fig:similarNonMatchCells_pltLabeled} shows an aligned cell 6, 1 between two non-match scans K013sA1 and K002eG1.
...
However, the bottom of \autoref{fig:similarNonMatchCells_pltLabeled} depicting the comparison plot between the aligned cell 6, 1 pair demonstrates that there are actually local similarities.
From the element-wise average we see that there are similar purple and orange regions shared between these two cells.
% We also see in the third column that there are large regions where the two surfaces differ.
% \jz{[Need to punch this section up more. The primary takeaway here is that scans may look different at the global scale but locally similar, so we need to consider both similarities AND differences.]}

This example underscores the need to analyze both similarities \textit{and} differences when comparing two scans.
...
<<neghborhoodLabeling_code,include=FALSE,eval=FALSE>>=
knownMatch_fullScan <- readRDS("data/comparisonResults_fullScan.rds")
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.8\textwidth]{images/similaritiesDifferencesRatio.png}
\caption{We compute the ratio between the number of similar and different elements of two aligned scans, which are defined to be elements for which the two surfaces are at most or greater than 1 micron, respectively. These are represented above as white pixels in the two images on the left. We then count the number of white pixels in each image and compute their ratio, resulting in this example in a value of 2.84. We expect this ratio to be larger for matching comparisons, on average, than non-matching comparisons.}
\label{fig:similaritiesDifferencesRatio}
\end{figure}
...
\autoref{fig:similaritiesDifferencesRatio} shows an example of this ratio computed for the matching pair K013sA1 and K013sA2, which results in a value of 2.84 meaning there are almost three times as many similarities as there are differences between the two scans.
...
We also compute similarities vs. differences ratios for the results of the cell-based comparison procedure outlined in \autoref{alg:cellComparison}.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figures/filterLabeling.png}
...
For example, the left side of \autoref{fig:filterLabeling} shows the matrix $|A - B^*| > 1$ for $A$ = K013sA1 and $B$ = K013sA2 with $TRUE$ elements represented in white.
We then use a connected components labeling algorithm detailed in \citet{Hesselink2001} and implemented in \citet{imager} to identify connected "neighborhoods" of $TRUE$ elements.
...
The middle of \autoref{fig:filterLabeling} shows each $S_{d,l}$ distinguished by different fill colors, $l = 1,...,L_d$.

The right side of \autoref{fig:filterLabeling} shows a histogram of the region sizes, denoted $|S_{d,l}|$ for $l = 1,...,L_d$.
We see that most of the sizes are relatively small, which agrees with our initial assumption.
There are a handful of larger regions that, when cross-referenced with the surface values in the original scans (see \autoref{fig:complementCompPlot_fullScan}, for example), are clearly different from each other, meaning the visual diagnostic works as intended.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.7\textwidth]{images/matchingCellDifferenceCorrelation.png}
...
\label{fig:matchingCellDifferenceCorrelation}
\end{figure}
...
For example, \autoref{fig:matchingCellDifferenceCorrelation} shows regions of two cells from a comparison between matching scans $A$ = K013sA1 and $B$ = K013sA2.
...
\autoref{fig:matchingCellDifferenceCorrelation} shows an example of computing the correlation between cell 2, 8 from scan K013sA1 and its mate in K013sA2.
...
\section{Statistical Learning from Visual Diagnostics}
...
We consider a data set of 210 cartridge cases scanned at the Roy J. Carver High Resolution Microscopy Facility at Iowa State University that were collected as part of a study by \citep{Baldwin}.
...
See \jz{[ACES paper]} for more information on these cartridge case data.

\subsection{Visual Diagnostic Statistics as Features}

Let $A$ and $B$ denote cartridge case scans.
We first perform the registration procedure of \autoref{alg:registration} in both comparison directions using a rotation grid of $\pmb{\Theta} = \{-30^{\circ} , -27^{\circ}, ..., 27^{\circ}, 30^{\circ}\}$, resulting in estimated full scan registrations $(m_{A}^*, n_{A}^*, \theta_A^*, CCF_{\max,A})$ and $(m_{B}^*, n_{B}^*, \theta_B^*, CCF_{\max,B})$.
Using these registrations, we obtain the aligned versions of the target scans; $B^*$ for $d = A$ and $A^*$ for $d = B$.

Next, we perform the cell-based comparison procedure of \autoref{alg:cellComparison} using rotation grids $\pmb{\Theta}_d' = \{\theta_d^* - 2^{\circ}, \theta_d^* - 1^{\circ},\theta_d^*,\theta_d^* + 1^{\circ}, \theta_d^* + 2^{\circ}\}$ for $d = A,B$ in both comparison directions, resulting in cell-wise registration sets $\pmb{F}_A$ and $\pmb{F}_B$.
...
However, we've seen in the example of \autoref{fig:similarNonMatchCells_pltLabeled} that even non-matching scans can share local similarities, in which case full scan features would prove more useful.
% We expect that the combination of these two scales will be more informative than either one separately.
...
\autoref{fig:visualFeaturePairs} shows a "generalized pairs plot" \citep{gpp, ggally} of the 9 visual diagnostic features distinguished by the ground-truth nature of the comparisons for the 21,945 training comparisons.
...
\jz{[More to say about this specific outlier? Look up example and talk about why it's an outlier]}

\jz{[More to say about univariate summaries?]}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{images/featureDensity_visualDiag_pairs.png}
\caption{Pairs plot for visual diagnostic feature values computed for 2,189 matching and 19,756 non-matching pairwise comparisons.}
\label{fig:visualFeaturePairs}
\end{figure}
...
Using the 21,945 training comparisons, we train three binary classifier models: based on a decision tree \citep{Breiman2017,rpart}, random forest \citep{Breiman2001,randomForest}, and logistic regression \citep{stats}.
...
See \jz{[ACES paper]} for a broader exploration of cartridge case similarity scoring algorithms.
...
\autoref{fig:visualDiag_rocPlt} shows ROC curve and AUC results for the 27 classifiers.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.7\textwidth]{images/visualDiag_rocPlt.png}
...
\label{fig:visualDiag_rocPlt}
\end{figure}

Consider the distributions of training match probabilities shown in \autoref{fig:visualDiagProb_density}, distinguished by firearm ID pair.
...
In contrast, matching comparisons from Firearm Z exhibit poorer separation in these important features compared to non-match comparisons, which explains the lower match probabilities in \autoref{fig:visualDiagProb_density}.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{images/visualDiagProb_density.png}
...
\label{fig:visualDiagProb_density}
\end{figure}
...
The phenomenon of classifier models not being as adept at identifying matching comparisons has been observed in many other cartridge evidence scoring methods \citep{song_proposed_2013,chen_convergence_2017}, \jz{[ACES paper]}.
...
For example, when applying the cell-based comparison procedure of \autoref{alg:cellComparison}, large markings in the target scan seem to "attract" source cells, even if those cells do not contain similar markings when visually compared.
Diagnostic tools like the X3P and Comparison plots are useful for understanding why a cell registers in these areas.

We return to the pair of matching cartridge cases shown in \autoref{fig:preProcessEffectExample}.
Recall that we computed the cross-correlation function (CCF) between these scans before and after extraneous, non-breech face observations were left in the scans.
...
The left side of \autoref{fig:firearmG_preprocessAlignedCell} shows the cell-wise registrations for this comparison using K002eG2 as reference and K227iG3 as target.
Similar to \autoref{fig:preProcessEffectExample}, the top and bottom show results from two cases: before and after removing the non-breech face observations from the two scans.
...
As a specific example, we we depict the comparison plot for cell 3, 4 with its aligned mate in the target scan on the right side of \autoref{fig:firearmG_preprocessAlignedCell}.
...
The first row of \autoref{fig:extremeProbExampleScans} shows matching K011sR1 and K046uR2 that have an estimated similarity score of 0.59.
The second row of \autoref{fig:extremeProbExampleScans} shows non-matching K013pC1 and K027gA3 that have an estimated similarity score of 0.38.
...
<<include=FALSE>>=
K011sR1 <- x3ptools::x3p_read("data/K011sR1.x3p")
K046uR2 <- x3ptools::x3p_read("data/K046uR2.x3p")
K013pC1 <- x3ptools::x3p_read("data/K013pC1.x3p")
K027gA3 <- x3ptools::x3p_read("data/K027gA3.x3p")
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=.7\textwidth]{figures/extremeProbExampleScans.png}
...
\label{fig:extremeProbExampleScans}
\end{figure}

As further support for the middling similarity scores for these two pairs, consider \autoref{fig:visualDiag_extremeProb_featureDensities} that shows numerical feature and score values for the 21,945 comparisons considered in the last section.
On the left, we visualize the densities for the 9 visual diagnost feature values - these are the same as the densities shown in \autoref{fig:visualFeaturePairs}.
On the right, we visualize the similarity score densities - this is a combination of the densities shown in \autoref{fig:visualDiagProb_density}.
On top of each density plot, we visualize the value associated with the two pairs considered in \autoref{fig:extremeProbExampleScans} as an orange (match) and black (non-match) line.
...
For example, the lower-diagonal of the pairs plot in \autoref{fig:visualFeaturePairs} shows the pairwise relationship of some features differs for matching vs. non-matching comparisons.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{images/visualDiag_extremeProb_featureDensities.png}
\caption{Density plots of 9 visual diagnostic features (left) and estimated similarity score (right) for 21,945 pairwise comparisons, distinguished by match and non-match comparisons. On top of each plot, we visualize the values associated with the two cartridge case pairs shown in \autoref{fig:extremeProbExampleScans}. We see that the feature values for these two pairs fall close to the intersection of the match and non-match densities, which suggests the cartridge cases are fairly unexceptional. This explains the middling similarity scores observed in the right plot.}
\label{fig:visualDiag_extremeProb_featureDensities}
\end{figure}

\subsection{Sensitivity to Filter Threshold}
...
\jz{[Final results don't seem heavily impacted using different multiples of the absolute difference standard deviation. Interestingly, the importance measure of the 9 features rearrange when we use the standard deviation of the "pooled" surface values.]}

\subsection{Interactive cartridgeInvestigatR application}
...
\jz{[Algorithms rarely have built-in mechanisms to determine when they work as expected. Diagnostic tools fill this gap. Visual diagnostics specifically provide intuitive ways to interpret the behavior of the algorithm. The visual diagnostics we developed can be used both as a quick reference to determine whether changes to earlier stages of the pipeline are warranted and as a tool to carefully study the behavior of the algorithm.]}

\jz{[We note that the workflow of dealing with such cartridge cases in practice would be iteratively applying pre-processing steps followed by using the visual diagnostic tools to ensure that the pre-processing removes as much extraneous information from the scans as possible.]}
