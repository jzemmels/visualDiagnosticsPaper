...
\jz{Mathematical details of the X3P Plot (mapping quantiles of surface values to a divergent color scale)}

\jz{Discuss how the X3P Plot helps in quickly identifying values that will unduly affect the registration procedure.}

\jz{Show example of removing the extreme values and the affect it has on the color scale}

\jz{Show example of pre-processing scans to remove }
